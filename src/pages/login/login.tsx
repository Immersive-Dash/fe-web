    ...
    axios
      ...
      .then((response) => {
        if (response.status === 200) {
          ...
          setStatus(false);
          Cookies.set('account', JSON.stringify(response.data.data), {
            expires: 1 / 24,
          });
        }
      })
        ...
        <div className="w-full bg-gray-50 rounded-lg shadow-lg md:mt-0 sm:max-w-md xl:p-0">
          <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
            <div className="space-y-3">
              ...
              <p className="text-sm text-[#423E3E] text-center">
                Enter your credentials to acess your account
              </p>
            </div>
            <form onSubmit={handleSubmit} className="space-y-4 md:space-y-6">
              <div>
                <label className=" mb-2 text-sm font-semibold text-[#5d5c5c]">
                  Email
                </label>
                <input
                  type="email"
              ...
              <div className="flex justify-center">
                <button
                  type="submit"
                  className="font-semibold text-white bg-[#3E31DF] hover:bg-[#03034F] rounded-full text-sm px-16 py-3 text-center"
                >
                  {status ? <Spinner /> : 'Login'}
                </button>
              </div>
            </form>
          </div>
